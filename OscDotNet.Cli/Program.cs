...
namespace OscDotNet.Cli
{
  internal class Program
  {
    private static void Main(string[] args)
    {
      ...
      if (int.TryParse(strport, out var temp)) port = temp;
      ...
      if (isServer)
      {
        ...
        while (value.ToLower() != "q") value = Console.ReadLine();
      }
      else
      {
    ...
    private static void TestMessageToBytes()
    {
      ...
      Debug.Assert(message.Equals(newmessage), "Roundtrip messages are not equal.");
      PrintMessage(newmessage);
    }

    private static void TestBytesToMessage()
    {
      var b = new byte[1000];
      var ival = BitConverter.GetBytes(123);
      var fval = BitConverter.GetBytes(321.123f);
    ...
    private static void PrintMessage(Message message)
    {
      ...
      foreach (var atom in message) Console.WriteLine("  => {0}", atom.ToString());
    }

    private static string GetTypeTagsString(TypeTag[] tags)
    {
